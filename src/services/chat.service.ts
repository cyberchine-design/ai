...
export class ChatService {
  static async sendChatMessage(
    ...
    logger.info(`Sending request to model ${payload.model} with ${inputTokens} input tokens (${transformedMessages.length} msgs, multimodal-aware).`);
    try {
      // Route to correct provider based on model name
      let apiUrl = env.MINIMAX_BASE_URL.replace(/\/$/, '');
      let apiKey = env.MINIMAX_API_KEY;
      
      if (payload.model && payload.model.toLowerCase().includes('agnes')) {
        apiUrl = env.AGNES_BASE_URL.replace(/\/$/, '');
        apiKey = env.AGNES_API_KEY;
        logger.info(`Routing to Agnes API: ${apiUrl}`);
      } else if (payload.model && payload.model.toLowerCase().includes('deepseek')) {
        apiUrl = env.DEEPSEEK_BASE_URL.replace(/\/$/, '');
        apiKey = env.DEEPSEEK_API_KEY;
        logger.info(`Routing to DeepSeek API: ${apiUrl}`);
      } else if (payload.model && payload.model.toLowerCase().includes('byteplus')) {
        apiUrl = env.BYTEPLUS_BASE_URL.replace(/\/$/, '');
        apiKey = env.BYTEPLUS_API_KEY;
        logger.info(`Routing to BytePlus API: ${apiUrl}`);
      } else if (payload.model && payload.model.toLowerCase().includes('gemini')) {
        apiUrl = env.GEMINI_BASE_URL.replace(/\/$/, '');
        apiKey = env.GEMINI_API_KEY;
        logger.info(`Routing to Gemini API: ${apiUrl}`);
      }
      ...
      if (!response.ok) {
        ...
        logger.error(`${payload.model} API request failed: ${response.status} - ${errorText}`);
        throw new Error(`${payload.model} API error: ${response.statusText} | ${errorText.substring(0, 500)}`);
      }
      ...
      logger.error(`Error invoking ${payload.model} API: ${error.message}`);
      throw error;
    }
  }
}
